jQuery(document).ready(function($) {
    ...
    var category_items_title = "";
    $('.category_items').click(function () {
        var cat_id = $(this).attr('id');
        $('.category-items a[data-id="' + cat_id + '"]').click();
        category_items_title = $(this).find('.category_items_title').text();
        $('#breadcrumbs').html("<a href='#' onclick='showall()'>"+category_items_title+"</a>");
    });
    ...
    //    $('.primary-category li a').removeClass('selected').find('span.arrow').remove();
      //  $(this).addClass('selected').append('<span class="arrow"></span>');
        ...
        if($(this).parents('.choose-category').find('.column-category[data-parent="'+clickedParentId+'"]').length == 0 ){
            $('#success-selection').show().data({'selectedId':clickedParentId,'selectedText':$(this).text()});
    ...
    $(document).on('click','.column-subcategory-wrapper .column-category li a',function (e) {
        ...
        $('#categortNextbutton').hide();
        $('#close-modal').show();
        var parent = $(this).closest('.column-category');
        $('.column-category').css('background',"#fff");
        $('.column-category').css('border-color',"#e1e1e1");
        
         
        parent.css("background","#e5f0ff");
        parent.css("border-color","#b9d6ff");
        ...
        if($(this).parents('.choose-category').find('.column-category[data-parent="'+clickedParentId+'"]').length == 0 ){
            ...
            $('.final_items').css('display','');
            $('.column-category').css('background',"#fff");
            $('.column-category').css('border-color',"#e1e1e1");
          //  $('#success-selection').click();
        }
        
        var breadcrumb_text = "<a href='#' onclick='showall()'>"+category_items_title+"</a>";
        
         $('.column-category li a.selected').each(function(){
            var text = $(this).text();
            breadcrumb_text += " > " + text;
         });
         
         $('#breadcrumbs').html(breadcrumb_text);
    });

    $(document).on('click','#success-selection',function (e) {
        e.preventDefault();
       ...
       // $('#success-selection').hide();
        ...
        $.post($('#category-fields').data('url'),{
            ...
            if(json.html) {
                ...
                $('#categortNextbutton').show();
                $('#listing_details').addClass('active');
                $('select').select2({
                    width: '100%',
                    language: site.language,
                    dir: site.dir,
