...
class ListingController extends Controller
{
    ...
    {   
        ...
        $mainImage = isset($ad->mainImage) && $ad->mainImage !== '' ? $ad->mainImage->image_path : '';
    ...
    public function actionUpdate($slug)
    {
        ...
        $breadCumData2 = '';
        $breadCumData3 = '';
        $categoryData3 = Category::find()->where(['category_id'=>$ad->category_id])->asArray()->one();
        if(!empty($categoryData3)){
            $breadCumData3 = !empty($categoryData3['name'])?$categoryData3['name']:'';
            $categoryData2 = Category::find()->where(['category_id'=>$categoryData3['parent_id']])->asArray()->one();
            if(!empty($categoryData2)){
                $breadCumData2 = !empty($categoryData2['name'])?$categoryData2['name']:'';
                $categoryData3 = Category::find()->where(['category_id'=>$categoryData2['parent_id']])->asArray()->one();
                $breadCumData1 = !empty($categoryData3['name'])?$categoryData3['name']:'';
            }
        }

        $breadCumData = '';
        if(!empty($breadCumData1)){
            $breadCumData = '<a href="#" onclick="showall()">'.$breadCumData1.'</a>>'.$breadCumData2.'>'.$breadCumData3;
        }
    ...
    public function actionPackage($slug)
    {
        ...
        if (request()->post()) {
            ...
            $loginId = Yii::$app->user->identity->user_id;
            $totalAds = 0;
            $ListingPackageUser = ListingPackageUser::find()->where(['user_id' => $loginId])->one();
            if(!empty($package->title != 'Free')){
                
                if(!empty($ListingPackageUser)){
                    if($ListingPackageUser->remaining_ads == 0){
                        $totalAds = $package->total_ads;
                    }else{
                        if($ListingPackageUser->package_id != $packageId){
                            $totalAds = $ListingPackageUser->remaining_ads+$package->total_ads;
                        }else{
                            $totalAds = $ListingPackageUser->remaining_ads;
                        }
                    }
                    
                }
                if(empty($ListingPackageUser)){
                    $ListingPackageUser = new ListingPackageUser();
                    $totalAds = $package->total_ads;
                }
                
                $ListingPackageUser->user_id = !empty($loginId)?$loginId:0;
                $ListingPackageUser->package_id  = !empty($packageId)?$packageId:0;
                $nowTotal =$totalAds-1;
                if($nowTotal < 0){
                    $ListingPackageUser->remaining_ads = 0;
                }else{
                    $ListingPackageUser->remaining_ads = $nowTotal;
                }
                $ListingPackageUser->save(false);
            }
