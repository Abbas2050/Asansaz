...
class AccountController extends Controller {
    ...
    public function actionMyListings() {
        ...
        $totalAds = Listing::find()->where(['customer_id' => app()->customer->identity->id])->count();
        $myPackage = ListingPackageUser::find()->where(['user_id' => app()->customer->identity->id])->with('package')->asArray()->one();
        if($myPackage !== '')
        {
            $totalAddOfPackageId= Listing::find()->where(['customer_id' => app()->customer->identity->id])->andWhere(['package_id'=>$myPackage['package']['package_id']])->count();
        }
        $totalMessage = Conversation::find()->where('status = :status AND (seller_id = :customer_id OR buyer_id = :customer_id)', [
                'status'      => Conversation::CONVERSATION_STATUS_ACTIVE,
                'customer_id' => app()->customer->identity->id,
        ])->count();
        $totalfavorites = ListingFavorite::find()->Where(['customer_id' => app()->customer->identity->id])->count();
        ...
        $myAdsProviderOnline = new ActiveDataProvider([
                                                              'query'      => Listing::find()->where(['customer_id' => app()->customer->identity->id, 'status' => 'active']),
                                                              'sort'       => ['defaultOrder' => ['listing_id' => SORT_DESC]],
                                                              'pagination' => [
                                                                      'defaultPageSize' => self::MYADS_PER_PAGE,
                                                              ],
                                                      ]);
        $myAdsProviderUnpublished = new ActiveDataProvider([
                                                                   'query'      => Listing::find()->where(['customer_id' => app()->customer->identity->id])->andWhere(['<>', 'status', 'active']),
                                                                   'sort'       => ['defaultOrder' => ['listing_id' => SORT_DESC]],
                                                                   'pagination' => [
                                                                           'defaultPageSize' => self::MYADS_PER_PAGE,
                                                                   ],
        ...
        return $this->render('my-listings', [
                'myAdsProvider' => $myAdsProvider,
                'totalAds' => $totalAds,
                'totalMessage' => $totalMessage,
                'totalfavorites' => $totalfavorites,
                'myAdsProviderOnline' => $myAdsProviderOnline,
                'myAdsProviderUnpublished' => $myAdsProviderUnpublished,
                'myPackage'=>$myPackage,
                'totalPackageAds'=>$totalAddOfPackageId
        ]);
    }
        ...
        if ($store->load(request()->post())) {
            ...
            try {
                ...
                if ($groupId == 1) {
                    ...
                    $store->logo = UploadedFile::getInstance($store, 'company_logo');
                    $store->logo->saveAs('uploads/images/store/' . $id . "_logo_" . $store->logo->baseName . '.' . $store->logo->extension);

                    $store->companyBanner = UploadedFile::getInstance($store, 'company_banner');
                    $store->companyBanner->saveAs('uploads/images/store/' . $id . "_banner_" . $store->companyBanner->baseName . '.' . $store->companyBanner->extension);


                    $store->attributes = request()->post('CustomerStore');
                    $store->customer_id = $id;
                    $store->company_logo = "/uploads/images/store/" . $id . "_logo_" . $store->logo->baseName . '.' . $store->logo->extension;
                    $store->company_banner = "/uploads/images/store/" . $id . "_banner_" . $store->companyBanner->baseName . '.' . $store->companyBanner->extension;
    ...
    public function actionJoin() {
        ...
        if ($emailExist) {
            notify()->addError(t('app', 'Duplicate Email Found'));
    ...
    public function actionSendPhoneCode() {
        ...
        $username = "923336781858";///Your Username
        $password = "ABBAS2050";//Your Password
        $mobile = $_POST['phone'];///Recepient Mobile Number
        $sender = "Pakistan";
        $message = $_POST['code'];;
