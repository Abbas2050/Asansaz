...
$sql_u = "SELECT total_views FROM ea_listing_stat  WHERE listing_id = '$listingid'";
    $responce=mysqli_query($conn,$sql_u);
    $row=mysqli_fetch_array($responce);
       $val = $row['0']+1;

$sql = "UPDATE ea_listing_stat SET total_views = '$val' WHERE listing_id = '$listingid'";
if ($conn->query($sql) === TRUE) {

         $index['tv'] = $row['0'];
         $index['messege'] = "View Added Successfully";
