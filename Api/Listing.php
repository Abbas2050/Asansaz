...
INNER JOIN ea_customer ON ea_listing.customer_id = ea_customer.customer_id)WHERE ea_listing.category_id = '$listid' ";
...
while($row=mysqli_fetch_array($responce))
    {
        ...
        while($rr = mysqli_fetch_array($res)){
            $i = $rr['0'];
            $index['imagepath']= "https://bahriamall.pk".$i;
        }
         $index['name']=  utf8_encode($row['0']);
        $index['listingid']= utf8_encode($row['1']);
        $index['price']= utf8_encode($row['3'].$row['2']);
        ...
        $index['customername']= utf8_encode($row['10'].' '.$row['11']);
        ...
        $index['status']= utf8_encode($row['17']);
